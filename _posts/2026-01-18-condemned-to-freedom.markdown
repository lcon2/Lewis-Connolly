...
description: Freedom in virtual worlds becomes a design problem: when nothing binds, identity thins and meaning drains. I argue that commitment and stakes, not suffering,
---
...
<style>
.audio-card { max-width: 600px; margin: 30px auto 20px; background: #1a1a1a; border: 1px solid #333; border-radius: 14px; padding: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.35); }
...
<div class="audio-card" data-player>
  <div class="audio-header">
    ...
    <div class="audio-volume">
      <label for="freedom-volume">Volume</label>
      <input id="freedom-volume" type="range" min="0" max="1" step="0.01" value="0.8" data-action="volume">
    </div>
  </div>
  <div class="audio-list">
    <button class="audio-track is-active" type="button" data-index="0">
      <span class="track-num">1</span>
      <span class="track-name">Rewind the Hour</span>
      <span class="track-time" data-time="0">--:--</span>
    </button>
  </div>
  <audio id="condemned-player" preload="none"></audio>
</div>

<script>
(function () {
  var player = document.getElementById('condemned-player');
  if (!player) return;
  var track = { title: 'Rewind the Hour', src: '/assets/audio/Rewind%20the%20Hour.mp3' };
  ...
  var timeEl = document.querySelector('[data-time="0"]');
  var isSeeking = false;
