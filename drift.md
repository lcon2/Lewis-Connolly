...
<style>
  ...
  /* Button centered */
  #beginBtn { position: fixed; left: 50%; top: 50%; transform: translate(-50%, -50%);
...
<!-- Video container (YouTube or branch) -->
<div id="video-container" style="position: fixed; inset: 0; opacity: 0; z-index: 0;"></div>
...
function onYouTubeIframeAPIReady(){
  ytPlayer = new YT.Player('video-container', {
    videoId: 'RmKkHZ-7rcY', width: '100%', height: '100%',
...
btn.addEventListener('click', ()=>{
  // fade audio
  let i=0, fadeIv = setInterval(()=>{ i++; bgAudio.volume *= 1 - i/20; if(i>=20){ clearInterval(fadeIv); bgAudio.pause(); } },50);
  // reveal video
